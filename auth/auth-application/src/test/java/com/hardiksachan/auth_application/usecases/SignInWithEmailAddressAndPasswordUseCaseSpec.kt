...
class SignInWithEmailAddressAndPasswordUseCaseSpec : FunSpec({
    val mockAuthFacade: AuthFacade = mockk()

    lateinit var sut: SignInWithEmailAddressAndPasswordUseCase

    beforeTest {
        sut = SignInWithEmailAddressAndPasswordUseCase(mockAuthFacade)
    }
    ...
    context("execute called") {
        test("call is forwarded to auth facade") {
            ...
            coEvery {
                mockAuthFacade
                    .signInWithEmailAndPassword(capture(emailSlot), capture(passwordSlot))
